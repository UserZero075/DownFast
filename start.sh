...
mostrar_ayuda() {
    ...
    echo
    echo "Uso:"
    echo "  bash start.sh           - Buscar y usar la ultima version disponible"
    echo "  bash start.sh -a        - Mostrar menu avanzado con opciones"
    echo "  bash start.sh -v<NUM>   - Descargar y ejecutar version especifica"
    echo "  bash start.sh -h        - Mostrar esta ayuda"
    echo
    echo "Ejemplos:"
    echo "  bash start.sh -v2.8.9   - Ejecutar version 2.8.9"
    echo "  bash start.sh -v1.5.0   - Ejecutar version 1.5.0"
    echo
    echo "====================================="
    echo
}

# Función para manejar versión específica
manejar_version_especifica() {
    local version="$1"
    local filename="index_${version}.js"
    ...
    imprimir_mensaje "INFO" "$AZUL" "Version especifica solicitada: v$version"
    
    # Verificar si el archivo ya existe localmente
    if [ -f "VPN/$filename" ]; then
        imprimir_mensaje "OK" "$VERDE" "Version v$version encontrada localmente"
        FILE_TO_EXECUTE="$filename"
        ejecutar_archivo
        return
    fi
    
    # Verificar si la versión existe en GitHub
    imprimir_mensaje "INFO" "$AZUL" "Verificando disponibilidad en GitHub..."
    
    local api_url="https://api.github.com/repos/UserZero075/DownFast/contents"
    local temp_file="$HOME/.github_temp_$$"
    
    if ! wget -q --timeout=30 --user-agent="DownFast-Updater" -O "$temp_file" "$api_url" 2>/dev/null; then
        imprimir_mensaje "ERROR" "$ROJO" "No se pudo conectar a GitHub"
        rm -f "$temp_file"
        exit 1
    fi
    
    # Verificar si la versión específica existe
    if grep -q "\"name\":\"$filename\"" "$temp_file" 2>/dev/null; then
        imprimir_mensaje "OK" "$VERDE" "Version v$version disponible en GitHub"
        rm -f "$temp_file"
        ...
        if descargar_archivo_github "$filename" "VPN/$filename"; then
            ...
            imprimir_mensaje "ERROR" "$ROJO" "Error al descargar la version v$version"
            exit 1
        fi
    else
        imprimir_mensaje "ERROR" "$ROJO" "La version v$version no existe en el repositorio"
        
        # Mostrar versiones disponibles
        imprimir_mensaje "INFO" "$AMARILLO" "Versiones disponibles:"
        local github_files=$(grep -o '"name":"index_[^"]*\.js"' "$temp_file" 2>/dev/null | sed 's/"name":"//g; s/"//g' | sed 's/^index_\|\.js$//g' | sort -V)
        
        if [ -n "$github_files" ]; then
            echo "$github_files" | while read -r ver; do
                echo "  - v$ver"
            done
        fi
...
case "$1" in
    -h|--help)
        ...
        ;;
    -a|--advanced)
        # Modo avanzado (funcionalidad de advStart.sh)
        echo
        echo "====================================="
        echo
        echo "     DownFast Auto-Updater v2.0"
    ...
    -v*)
        ...
        if [ -z "$version" ]; then
            imprimir_mensaje "ERROR" "$ROJO" "Debe especificar un numero de version. Ejemplo: -v2.8.9"
            mostrar_ayuda
            exit 1
        fi
        ...
        # Manejar versión específica
        manejar_version_especifica "$version"
        ;;
    "")
        # Modo por defecto (funcionamiento original)
        echo
        echo "====================================="
        echo
        echo "     DownFast Auto-Updater v2.0"
        ...
        ;;
    *)
        imprimir_mensaje "ERROR" "$ROJO" "Opcion no reconocida: $1"
        mostrar_ayuda
        exit 1
        ;;
esac
