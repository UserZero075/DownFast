...
while true; do
    ...
    # Lanzar slipstream-client y capturar su salida
    ./slipstream-client \
    ...
    # Vigilar el proceso: si muere, si no hay raw bytes, o si llega el tiempo programado
    while [ "$(date +%s)" -lt "$end_ts" ]; do
        if ! kill -0 "$PID" 2>/dev/null; then
            echo ""
            echo "[$(date '+%H:%M:%S')] slipstream-client se cayó. Reconectando..."
            rm -f "$LAST_RAWBYTES_FILE"  # Resetear para el próximo ciclo
            sleep "$RETRY_DELAY"
            break
        fi

        # Verificar inactividad de raw bytes
        if [ -f "$LAST_RAWBYTES_FILE" ]; then
            last_rawbytes=$(cat "$LAST_RAWBYTES_FILE")
            now=$(date +%s)
            inactivity=$((now - last_rawbytes))
            
            if [ "$inactivity" -ge "$INACTIVITY_TIMEOUT" ]; then
                echo ""
                echo "[$(date '+%H:%M:%S')] Sin raw bytes por ${inactivity}s. Reiniciando..."
                if kill -0 "$PID" 2>/dev/null; then
                    kill "$PID" 2>/dev/null
                    wait "$PID" 2>/dev/null
                fi
    ...
    # Si llegamos al tiempo programado, reiniciar
